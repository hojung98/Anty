...
class ChatFetcherApp(QWidget):
    def __init__(self):
        ...
        self.url_label = QLabel("영상 URL을 입력하세요:")
    ...
    def start_fetching(self):
        ...
        if not url or not nickname:
            self.chat_display.setText("❌ 영상 URL과 닉네임을 모두 입력하세요.")
            return

        match = re.search(r'/video/(\d{7})', url)
        if not match:
            self.chat_display.setText("❌ 유효한 영상 URL 형식이 아닙니다.")
            return
